...
function getBufferByWindow (buffers, winIn)
{
    if (!winIn)
        return null;

    var rootWindow = getRootWindow(winIn);
    // dump("[rootWindow]: " + rootWindow + "\n");
    
    if (rootWindow) {
        var count = buffers.count;
        for (let i = 0; i < count; ++i) {
            var buffer = buffers.get_buffer(i);
            if (buffer.browser.contentWindow == rootWindow)
                return buffer;
        }
    }
    return null;
...
function getRequestLoadContext(request)
{
    try {
        if (request && request.notificationCallbacks) {
            // dump("getRequestLoadContext ==> request && request.notificationCallbacks\n");
    ...
    try {
        if (request && request.loadGroup && request.loadGroup.notificationCallbacks) {
            // dump("getRequestLoadContext ==> request && request.loadGroup && request.loadGroup.notificationCallbacks\n");
            return request.loadGroup.notificationCallbacks.getInterface(Ci.nsILoadContext);
        }
    }
    catch (exc) {}

    // dump("getRequestLoadContext ==> null\n");
    return null;
}
